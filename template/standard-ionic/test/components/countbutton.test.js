...
describe('Count Button', () => {
  ...
  it('Label renders correctly', () => {
    expect(result.first("ion-button").innerHTML).toEqual(label + " is 100");
  })
  it('Count function works correctly', () => {
    result.click("ion-button");
    expect(result.first("ion-button").innerHTML).toEqual(label + " is 101");
  });
  it('Remove component', () => {
    result.onremove();
    expect(result.should.not.have("ion-button")).toBeTruthy;
  });
});